...
#define JOYSTICK_THRESHOLD_MS 500
...
{
	if (microej_time_getcurrenttime(MICROEJ_TRUE) - last_pressed_time >= JOYSTICK_THRESHOLD_MS)
	{
		BUTTONS_HELPER_pressed(joy_id);
		last_pressed_time = microej_time_getcurrenttime(MICROEJ_TRUE);
	}
}
